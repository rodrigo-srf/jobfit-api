...
def create_application(payload: ApplicationCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    if db.query(Application).filter(Application.job_id == payload.job_id, Application.owner_id == current_user.id).first():
        raise HTTPException(status_code=409, detail="Application already exists")
    application = Application(job_id=payload.job_id, owner_id=current_user.id, stage=payload.stage, notes=payload.notes)
    job.status = "applied"
...
def list_applications(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    return db.query(Application).filter(Application.owner_id == current_user.id).order_by(Application.updated_at.desc()).all()
...
def update_application(application_id: int, payload: ApplicationUpdate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    application = db.query(Application).filter(Application.id == application_id, Application.owner_id == current_user.id).first()
    if not application:
